...
async function main() {
  ...
  // Discontinuing a reference means deleting it here, not just leaving it
  // out of PRODUCTS -- otherwise the old row (and its /watches page) just
  // keeps existing untouched by every future reseed.
  const { count: discontinued } = await prisma.product.deleteMany({
    where: { slug: { notIn: PRODUCTS.map((p) => p.slug) } },
  });
  if (discontinued > 0) console.log(`Removed ${discontinued} discontinued product(s).`);
